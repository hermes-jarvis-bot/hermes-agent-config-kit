...
- `ai-ml/diffusion-engineering` (with reviewed architectures, encoders/data, evaluation/debugging, memory, samplers, and training references)
...
Reviewed-script lane: a narrow, human-gated exception to the quarantine lane for **skill-bundled** scripts only — a script that ships inside `hermes/skills/<domain>/<name>/scripts/` and is invoked by the operator or agent at runtime, exactly like Hermes's own official skills (e.g. `xlsx`, `pdf`, `linear`) ship and invoke their own bundled scripts. This is mechanically distinct from the quarantined top-level `hooks/`/adapter `scripts/`: a skill-bundled script is just a plain script a skill's markdown tells the operator to run, which Hermes already supports natively, whereas a Claude-Code hook has a harness-lifecycle I/O contract (stdin JSON, a stdout-JSON decision, `safety_common.py`). **Update, 2026-08-07**: that hook contract is no longer "no Hermes equivalent" — Hermes has a structurally equivalent shell-hook system (`~/.hermes/config.yaml`'s `hooks:` block, verified against the live `agent/shell_hooks.py` source), so a hook can now be deliberately reimplemented for it under the separate reviewed-hook lane — see `SECURITY.md`'s "Reviewed-hook lane" and `mappings/reviewed-hooks.yaml`. It is still never a direct copy: the two I/O contracts differ enough that a byte-identical port would silently misbehave (Hermes never blocks on exit code, only on stdout JSON).

A script may exist under `hermes/skills/**/scripts/` only if it is listed in `mappings/reviewed-scripts.yaml` with a `source_sha256` of the exact upstream fetch, and only after this gate has been satisfied by hand:

1. a full manual read of the entire script (not a grep-only pass);
2. a dependency check (stdlib preferred; any external dependency documented as an explicit prerequisite);
3. confirmation of no destructive filesystem operations, no credential/secret reads or logging, and no undisclosed network calls;
4. a path-portability check (no hardcoded Claude-Code-specific paths such as `.claude/`/`~/.claude/`, unless kept deliberately as documented upstream-source context);
5. a passing syntax check (`compile()`/`py_compile` for Python, the JS/TS equivalent for JS);
6. a passing run of this repo's own automated dangerous-pattern and secret scans (`validate_reviewed_scripts()` in `scripts/validate_output.py`);
7. never invoked automatically by this adapter or its CI — only ever invoked by an operator or agent at runtime, under Hermes's own normal permission/consent system.

A reviewed script is **never** auto-converted or auto-regenerated by `scripts/sync_upstream.py`'s `SUPPORTED`/`convert_supported()` fast lane (that pipeline is markdown-only and cannot safely reason about executable code). If the upstream source changes (its live SHA-256 no longer matches the recorded `source_sha256`), the change must be re-reviewed by hand before the manifest entry is updated — the same manual-reapproval discipline this adapter already applies to hand-adapted markdown sources.

A script that goes through this same gate and is turned down is recorded in `mappings/rejected-scripts.yaml` (path, `source_sha256`, reason, and a `revisit_condition`) rather than left as an undocumented omission — see `SECURITY.md`'s "Rejected scripts" section.
...
- verify quarantined upstream artefacts did not leak into `hermes/`, except paths explicitly allowlisted in `mappings/reviewed-scripts.yaml`;
- verify every allowlisted reviewed script passes a syntax check plus dangerous-pattern, secret-pattern, and provenance-marker checks (`validate_reviewed_scripts()`);
