...
Reviewed-script lane: a narrow, human-gated exception to the quarantine lane for **skill-bundled** scripts only — a script that ships inside `hermes/skills/<domain>/<name>/scripts/` and is invoked by the operator or agent at runtime, exactly like Hermes's own official skills (e.g. `xlsx`, `pdf`, `linear`) ship and invoke their own bundled scripts. This is mechanically distinct from the quarantined top-level `hooks/`/adapter `scripts/`: a Claude-Code hook has a harness-lifecycle I/O contract (stdin JSON, exit codes, `safety_common.py`) that has no Hermes equivalent and is not portable without a translation adapter; a skill-bundled script is just a plain script a skill's markdown tells the operator to run, which Hermes already supports natively.
