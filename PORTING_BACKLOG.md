...
Inventory from the current `upstream/claude-code-config/snapshot/`. The table between the two
`<!-- ... SNAPSHOT_BASELINE_TABLE ... -->` markers below is **generated, not hand-maintained** —
run `python3 scripts/update_backlog_counts.py` to refresh it after any upstream sync or any edit
to `mappings/{reviewed,rejected}-{hooks,scripts}.yaml`; `--check` exits non-zero without writing
if it has drifted (this is also wired into `sync_upstream.py --sync`, so a routine sync commit
already includes the refreshed table, and into `validate_output.py`, so CI fails loudly if the
table and the manifests it's derived from ever disagree — see "Automation" below). A prior
hand-recount undercounted `hooks/` Ported by one (missed `hermes_hook_common.py`, source
`hooks/safety_common.py`) — the generator reads the manifests directly instead of re-deriving
the list by hand each time.

The **Ported** column is the fast-lane (`sync_upstream.SUPPORTED`) count *plus* every
individually reimplemented artefact in `mappings/reviewed-hooks.yaml` and
`mappings/reviewed-scripts.yaml` (grouped by source-path prefix) — both lanes are a deliberate,
final "this is in `hermes/`" decision, just reached through different review paths (see "Ported
so far" below and `SECURITY.md`'s "Reviewed-hook lane" / "Reviewed-script lane"). The
**Rejected** column counts `mappings/rejected-hooks.yaml` / `mappings/rejected-scripts.yaml`
entries whose rejection is still current — one later reversed by its own `superseded_by` field
...
<!-- BEGIN SNAPSHOT_BASELINE_TABLE (auto-generated by scripts/update_backlog_counts.py — do not hand-edit) -->
...
| `rules/` | 35 | 29 | 0 | 6 |
| `scripts/` | 82 | 0 | 1 | 81 |
| `skills/` | 371 | 155 | 0 | 216 |
| `templates/` | 50 | 34 | 0 | 16 |
| `workflows/` | 5 | 0 | 0 | 5 |
| **Total** | **739** | **272** | **14** | **453** |
...
_Last recomputed against upstream commit `e2c14eff8397524dca7b609a3c99baf70216ff8c` (`upstream.lock.json`'s `last_synced_sha`)._

<!-- END SNAPSHOT_BASELINE_TABLE -->

## Automation

`scripts/update_backlog_counts.py` computes the table above from live state (snapshot file
counts, `sync_upstream.SUPPORTED`, and the four `mappings/*.yaml` manifests) and rewrites only
the region between the two markers. Two call sites make it self-maintaining:

- `sync_upstream.py --sync` calls it at the end of every sync, so the sync PR's own diff already
  carries the refreshed table — no separate manual step after merging.
- `validate_output.py` runs it with `--check` as part of the standard validation pass, so CI
  fails if a manual `mappings/reviewed-hooks.yaml` (etc.) edit ever changes Ported/Rejected
  without the table being regenerated to match.

Both call sites share the exact same computation as a plain `python3 scripts/update_backlog_counts.py`
run — there is no separate "official" recount procedure to keep in sync by hand.
...
65 files under `scripts/` alone (see the "Snapshot baseline" table above) — treat `ls
