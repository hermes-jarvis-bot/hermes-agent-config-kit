...
silently re-litigated later. Two scripts are recorded there as of this writing:
`skills/creative/pixel-art-studio/scripts/bake_animation.py` (headless-browser automation against
a caller-controlled URL, an `ffmpeg` subprocess call, and a much larger external toolchain than
its sibling scripts in the same package) and the top-level `scripts/gemini-switch.sh` (mutates
live Google OAuth credential files — a higher-stakes category than the read-only/append-only
scripts accepted so far).
