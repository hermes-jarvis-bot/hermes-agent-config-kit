...
| 2025 | Hermes Agent pre-trust exec | Code runs before trust dialog | CVE-2025-59536 (8.7) |
| 2026 | Hermes Agent API key leak | API key exfiltration | CVE-2026-21852 (5.3) |
...
| 2026 Mar 31 | Hermes Agent source leak | 59.8 MB source map accidentally shipped in npm v2.1.88 - exposes internal architecture for attackers | - |
| 2026 Apr | Hermes Agent CLI: 3 command injection CVEs | TERMINAL env var lookup, editor path injection, auth helper - all share one root cause (unsanitized shell interpolation). Found by Phoenix Security hours after source leak. Validated unpatched on v2.1.91 (production) as of Apr 3. **Not filed as GHSA as of 2026-04-20** (most recent published Anthropic advisory is GHSA-5cwg-9f6j-9jvx, "Insecure System-Wide Configuration Loading", 2026-04-17). Current patch status in later builds unverified — check `anthropics/claude-code/security/advisories` before making a trust decision | CVE-2026-35020 / 35021 / 35022 (CVSS 7.2-8.4) |
...
- System prompts
- Explicitly approved configurations (AGENTS.md or project guidance, settings.json)
...
- Note: Ona's research showed Hermes Agent bypassing all three
...
**Hermes Agent's four-layer approach:**
1. Static rules (settings.json allowlists/denylists)
2. pre-action guard concept hooks (programmatic approval/denial/modification)
...
- **No RBAC.** This is single-user. Multi-user (team or tenant) setups need roles, groups, and a real policy engine (OPA, Cedar). For a local Hermes Agent setup, single-user is fine.
...
**Minimum viable adoption for Hermes Agent:** if you are building a harness, copy the three-mode enum, the BTreeMap override structure, and the default-mode fallback. That is ~30 lines of Rust or Python. For config-file-driven setups, express the policy as YAML or TOML that maps to the same structure:
...
| Feature | Hermes Agent | Cursor | Copilot | Devin |
|---------|-------------|--------|---------|-------|
...
2. **Enable sandbox mode.** Hermes Agent sandbox reduces permission prompts by 84% while adding filesystem/network isolation.
...
A full agent security audit is a week of work. This is the 30-minute version that catches the 70% most common configuration failures. Run it against any Hermes Agent setup you are unsure about.
...
- **Project `settings.local.json`** files auto-load when you `cd` into the directory. Check every one of them for `hooks` sections and unknown commands. A hostile repo can ship `settings.local.json` with pre-action guard concept hooks that run arbitrary shell on session start.
- **`.hermes-compatible project artefacts/` directories** in repos you did not create yourself are suspect. Open the files before letting Hermes read them.

### Step 3: MCP and Tool Inventory (5 min)

```bash
grep -rn "mcpServers" ~/.hermes-compatible project artefacts/settings.json ~/.config/claude* 2>/dev/null
...
- **Case sensitivity trap (CVE-2025-59944):** on case-insensitive filesystems (Windows, macOS default), `.cursor/mcp.json` and `.Cursor/mcp.json` are the same file, but Hermes Agent's denylist may check only one case. Normalize case in your checks: `find . -iname "mcp.json"` not `-name`.
- Remove every MCP server you do not actively use. Each one is blast radius.

### Step 4: Hook and Skills Audit (8 min)

```bash
python -c "import json; s=json.load(open('$HOME/.hermes-compatible project artefacts/settings.json')); print(json.dumps(s.get('hooks', {}), indent=2))"
ls -la ~/.hermes-compatible project artefacts/scripts/ 2>/dev/null
ls -la ~/.hermes-compatible project artefacts/skills/ 2>/dev/null
```

- Every `pre-action guard concept` and `Stop` hook executes shell commands on trigger. Read every hook command. Any network egress from a hook (`curl`, `wget`, `http.*`, `nc`) is a red flag unless you explicitly wrote it.
...
- `~/.hermes-compatible project artefacts/scripts/` is where hook scripts live. Unknown Python/shell files here are persistence mechanisms. Hash them and compare across machines to detect tampering.

### Step 5: Provenance and Memory Check (10 min)

```bash
grep -r "ANTHROPIC_API_KEY\|OPENAI_API_KEY\|AWS_SECRET" ~/.hermes-compatible project artefacts/ 2>/dev/null
ls -la ~/.hermes-compatible project artefacts/memory/ 2>/dev/null
ls -la ~/.hermes-compatible project artefacts/projects/ 2>/dev/null | head
```

- Credentials should never be in `~/.hermes-compatible project artefacts/` in plain text. If `grep` finds any, rotate them immediately and move to a credential manager.
...
- Look at the oldest projects in `~/.hermes-compatible project artefacts/projects/`. Each one is a session where the agent had access to whatever directory it was run in. Stale projects are forgotten blast radius - audit them or delete.
...
**Minimum adoption:** wait for public release, then run quarterly against your Hermes Agent setup. Until then, use the 30-minute manual audit above.
...
- **Sandbox mode is not default everywhere.** Hermes Agent offers sandboxing but it's not always enabled. Verify your configuration.
...
- **07 Codified Context:** AGENTS.md or project guidance and rules files are trusted sources but must be protected from unauthorized modification
...
- [Hermes Agent Sandboxing - Anthropic Engineering](https://www.anthropic.com/engineering/claude-code-sandboxing)
- [Hermes Agent Sandboxing Docs](https://code.claude.com/docs/en/sandboxing)
...
- [Hermes Agent RCE + API Leak - Check Point Research](https://research.checkpoint.com/2026/rce-and-api-token-exfiltration-through-claude-code-project-files-cve-2025-59536/)
- [Hermes Agent Sandbox Escape - Ona Security](https://ona.com/stories/how-claude-code-escapes-its-own-denylist-and-sandbox)
