...
Source: AnastasiyaW/claude-code-config/skills/ai-ml/notebooklm-grounded-research/scripts/verify_notebooklm_setup.py
Ported unmodified after full manual review: stdlib-only (argparse, json, os, shutil,
sys, pathlib, tomllib), no network calls, no subprocess/os.system/eval/exec, no writes.
It only reads a local MCP client config file and checks filesystem path existence; it
explicitly never reads cookie/token/browser-database content (see secrets_read below).
Run it yourself and read it before trusting it.
"""
...
def verify() -> dict:
    ...
    return {
        ...
        "next_step": (
            "Run a live get_health call; if unauthenticated, ask the operator to run visible setup_auth."
            if config_ok
