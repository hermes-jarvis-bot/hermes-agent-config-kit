...
Source: AnastasiyaW/claude-code-config/skills/creative/pixel-art-studio/scripts/animate.py
Ported unmodified after full manual review: stdlib (argparse, json, math, sys, pathlib) plus
Pillow (documented prerequisite: pip install Pillow); imports this skill's own render.py
directly (sys.path insert, not a subprocess). No network calls, no subprocess/os.system/eval/
exec, no credential access. Writes only to user-supplied or clearly-named default output
paths. Run it yourself and read it before trusting it.
