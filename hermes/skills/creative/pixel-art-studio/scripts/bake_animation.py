...
Source: AnastasiyaW/claude-code-config/skills/creative/pixel-art-studio/scripts/bake_animation.py
This script was INITIALLY REJECTED after full manual review (see
mappings/rejected-scripts.yaml for that record) because it drove a headless Chromium
browser, via Playwright, to an arbitrary caller-supplied URL with no restriction, and
never cleaned up the temporary frame directory it created on every run. Operator review
approved two deliberate modifications (not present upstream) to close those gaps before
accepting the script, keeping everything else — the capture loop, the five encoders, the
CLI surface — unmodified:
  1. `_require_local_url()` (new): rejects any URL whose host is not localhost/127.0.0.1/
     ::1, called at the top of `main()` before anything else runs. This is the load-bearing
     fix — it stops the tool from ever driving the browser against a non-local target.
  2. `bake()`'s capture-and-encode body is now wrapped in try/finally so the `.bake_*`
     temporary frame directory is always removed after the run, success or failure.
Dependencies (external, documented prerequisite): pip install playwright Pillow;
playwright install chromium (one-time); ffmpeg in PATH (for the webm-alpha/mp4 formats).
Shells out to ffmpeg via subprocess.run() with an argv list (not shell=True) — no shell-
injection risk from that call. Run it yourself and read it before trusting it.
...
Usage:
    ...
    # url must be a local address (localhost/127.0.0.1/::1) — see _require_local_url()
...
# --- Local-URL guard (added by hermes-agent-config-kit, not in upstream) ----

LOCAL_HOSTNAMES = frozenset({"localhost", "127.0.0.1", "::1"})


def _require_local_url(url: str) -> None:
    """Reject any URL that does not point at a loopback address.

    This tool drives a headless browser to the given URL and executes whatever
    JavaScript that page contains. Without this check, a caller-supplied URL could
    point at an arbitrary external address. Restricting to loopback keeps the tool's
    actual purpose (baking a local dev-server preview) while removing that exposure.
    """
    parsed = urlparse(url)
    if parsed.scheme not in ("http", "https"):
        print(f"Error: URL must be http:// or https://, got scheme {parsed.scheme!r} in {url!r}",
              file=sys.stderr)
        sys.exit(1)
    hostname = (parsed.hostname or "").lower()
    if hostname not in LOCAL_HOSTNAMES:
        print(
            f"Error: refusing to open non-local URL {url!r}. This tool only accepts "
            f"http(s)://localhost, 127.0.0.1, or ::1 addresses (got host {hostname!r}), "
            f"since it drives a headless browser to run whatever JavaScript that page "
            f"contains.",
            file=sys.stderr,
        )
        sys.exit(1)
...
async def bake(url: str, canvas_id: str, period_ms: int, fps: int,
    ...
    """
    # Capture frames to a temp directory, always removed afterward (added by
    # hermes-agent-config-kit: upstream never cleaned this directory up).
