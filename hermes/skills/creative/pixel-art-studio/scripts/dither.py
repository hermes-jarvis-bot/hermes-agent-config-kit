...
Source: AnastasiyaW/claude-code-config/skills/creative/pixel-art-studio/scripts/dither.py
Ported unmodified after full manual review: stdlib (argparse, json, sys, pathlib) plus
Pillow and numpy (documented prerequisite: pip install Pillow numpy). No network calls, no
subprocess/os.system/eval/exec, no credential access. Reads bundled palette .hex files
(relative to this script) and the user-supplied input image; writes only to the
user-supplied -o/--output path. Run it yourself and read it before trusting it.
