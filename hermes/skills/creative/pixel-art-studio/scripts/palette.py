...
Ported unmodified after full manual review: stdlib (argparse, colorsys, json, sys, pathlib)
plus Pillow and numpy (documented prerequisite: pip install Pillow numpy). No network calls
anywhere (confirmed no requests/urllib/httpx/socket imports) — --extract, --search-tag, and
--mood all operate purely on local bundled data and local image files. No subprocess/os.system
/eval/exec, no credential access. Writes only to the -o/--output path a caller supplies. Run
it yourself and read it before trusting it.
