...
Source: AnastasiyaW/claude-code-config/skills/creative/pixel-art-studio/scripts/preprocess.py
Ported unmodified after full manual review: stdlib (argparse, json, sys, pathlib, tempfile)
plus Pillow and numpy (documented prerequisite: pip install Pillow numpy); imports this
skill's own dither.py/palette.py directly (sys.path insert, not a subprocess). No network
calls, no subprocess/os.system/eval/exec, no credential access. Writes one self-cleaning
temp PNG (created and unlinked in a finally block) plus the user-supplied -o/--output path.
