...
Source: AnastasiyaW/claude-code-config/skills/creative/pixel-art-studio/scripts/quality_check.py
Ported unmodified after full manual review: stdlib (argparse, json, math, sys,
collections.Counter, pathlib, typing) plus Pillow and numpy (documented prerequisite: pip
install Pillow numpy). Fully read-only: no network calls, no subprocess/os.system/eval/exec,
no credential access, and no file writes at all — only reads the input image/JSON and prints
JSON findings to stdout. Its nested per-pixel loops are O(H*W); a very large input image is a
CPU-time consideration, not a security one. Run it yourself and read it before trusting it.
