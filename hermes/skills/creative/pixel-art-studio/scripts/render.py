...
Source: AnastasiyaW/claude-code-config/skills/creative/pixel-art-studio/scripts/render.py
Ported unmodified after full manual review: stdlib (argparse, json, os, sys, pathlib, typing)
plus Pillow (documented prerequisite: pip install Pillow). No network calls, no subprocess/
os.system/eval/exec, no credential access. Reads a JSON spec (path or stdin) and bundled
palette .hex files (relative to this script); writes only to the -o/--output path a caller
supplies. Run it yourself and read it before trusting it.
