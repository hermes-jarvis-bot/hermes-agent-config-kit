...
metadata:
  hermes_config_kit:
    ...
    source_path: rules/no-guessing.md
...
Source: `AnastasiyaW/claude-code-config/rules/no-guessing.md`.
...
повторяет fresh evaluator. `a reviewed guard candidate` ловит в финальном
...
- **"Сначала документация, потом действия — НИКОГДА наугад"** в основном
  AGENTS.md or project guidance — это правило усиливает и расширяет тот принцип. Документация
...
Soft-enforce через session-start routine concept hook который при старте сессии в проекте
...
В практике для меня в Hermes Agent: **перед важным/необратимым действием**
...
1. **pre-action guard concept soft-warn**: для destructive ops без `# verifier-approved: <agent-id> <ts>` token — print reminder "Consider running independent verifier".
...
- **[verify-at-consumer.md](verify-at-consumer.md)** — специализация для integrations: правда об контракте живёт в коде получателя, не в spec doc или коде отправителя. Idea credit: a collaborator's parallel Hermes session, real case file_uploaded webhook 2026-04-28
