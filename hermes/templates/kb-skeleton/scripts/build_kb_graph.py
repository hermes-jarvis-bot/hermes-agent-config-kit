...
Source: AnastasiyaW/claude-code-config/templates/kb-skeleton/scripts/build_kb_graph.py
Ported unmodified after full manual review: stdlib only (argparse, json, re, sys,
collections.defaultdict, pathlib, typing, __future__). Fully read-only against the project
it is copied into except for its own designated output directory (docs/_graph/ under that
project's repo root): no network calls, no subprocess, no eval/exec, no credential access.
This is a template script meant to be copied into an operator's own project (see
templates/kb-skeleton/README.md), not invoked by this adapter itself. Run it yourself and
...
def _normalize_feature_id(raw: str) -> str | None:
    """Map both this script's own F-NNN doc-id convention (derived from
    feat-NNN-slug.md filenames) and the long-run-feature-tracking skill's feat-NNN
    feature_list.json convention to the same comparison key, so the two spellings
    of the same feature reconcile instead of showing as a false sync mismatch."""
    if not isinstance(raw, str):
        return None
    m = re.match(r"^(?:F-|feat-)(\d+)$", raw, re.IGNORECASE)
    if not m:
        return None
...
def gen_health_report(
    ...
    if feature_list_json_path and feature_list_json_path.is_file():
        ...
        if fl and isinstance(fl, dict) and "features" in fl:
            json_by_key = {
                _normalize_feature_id(item.get("id")): item.get("id")
                for item in fl["features"] if isinstance(item, dict)
            }
            json_by_key.pop(None, None)
            doc_by_key = {_normalize_feature_id(fid): fid for fid in all_features}
            only_in_json = set(json_by_key) - set(doc_by_key)
            only_in_docs = set(doc_by_key) - set(json_by_key)
            for key in sorted(only_in_json):
                lines.append(f"- [ERROR] feature_list.json has `{json_by_key[key]}` but no docs/layers/*/features/feat-*.md file found")
                errors += 1
            for key in sorted(only_in_docs):
                lines.append(f"- [WARN] {doc_by_key[key]} has a doc but is missing from feature_list.json")
                warnings += 1
    else:
