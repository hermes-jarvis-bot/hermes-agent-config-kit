...
Source: AnastasiyaW/claude-code-config/templates/kb-skeleton/scripts/validate_kb.py
Ported unmodified after full manual review: stdlib only (re, sys, pathlib, __future__).
Fully read-only against the project it is copied into: no network calls, no subprocess,
no eval/exec, no credential access. It only reads markdown/text files under docs/kb/ and
AGENTS.md and prints findings to stdout/stderr; it never writes any file. This is a template
script meant to be copied into an operator's own project (see templates/kb-skeleton/README.md),
not invoked by this adapter itself. Run it yourself and read it before trusting it.
"""
