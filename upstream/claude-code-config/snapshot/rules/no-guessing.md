...
## Внешнее утверждение = измерение, не чтение кода (2026-08-14)

Код объясняет **намерение реализации**, но сам по себе не доказывает байты
файла, фактический хеш, версию установленного пакета, состояние сервиса,
деплой или результат запроса. Поэтому перед сообщением человеку каждый
проверяемый внешний факт получает один из четырёх честных классов:

| Класс | Когда допустим | Формулировка |
|---|---|---|
| `OBSERVED` | Только что выполнен probe и виден его результат | "`Get-FileHash` вернул ..." |
| `DERIVED` | Вывод вычислен из уже измеренных входов; указана формула | "Из A и B следует C" |
| `HYPOTHESIS` | Есть лишь код, документация или косвенный признак | "По коду ожидается ...; не проверено" |
| `BLOCKED` | Проверка реально запускалась, но ей не хватает внешнего входа | "Не подтверждено: команда X упёрлась в Y" |

Нельзя превращать `HYPOTHESIS` в `OBSERVED` гладкой фразой вроде
"значит хеш совпадает" или "релиз задеплоен". Для hash/size/version/deploy,
контрольной суммы, сетевого статуса и любого численного результата внешнее
сообщение обязано содержать короткую связку:

```text
Claim: <точный факт>
Evidence: <команда/API/probe> -> <наблюдённый результат>
Scope: <что именно, где и когда проверено>
```

Если точного probe пока нет, не выдаём догадку за результат: либо запускаем
самый дешёвый достаточный probe, либо пишем `HYPOTHESIS`/`BLOCKED` и называем
конкретный отсутствующий вход. Для важного или необратимого утверждения probe
повторяет fresh evaluator. `outward-claim-evidence-guard.py` ловит в финальном
сообщении только узкий набор таких фактов; это страховка отчёта, а не подмена
самой проверки.

Почему это жёсткая граница, а не стилистическая рекомендация:

- [AnalysisBench](https://arxiv.org/abs/2604.11270) показывает, что
  самопроверка агента систематически завышает успех относительно ручной
  верификации.
- [SEAL](https://arxiv.org/abs/2607.24300) показывает, что самонаписанная
  проверка может оставаться почти идеальной, пока качество в реальном
  развёртывании падает; этот разрыв ограничивает фиксированный внешний сигнал
  приемки.
...
## 🔴 «Проверить не могу» — это утверждение, которое требует доказательства (2026-07-31)

Отказ от проверки — такое же утверждение, как любое другое, и подчиняется тому же правилу:
**не гадать о собственных возможностях.** Реальный случай: агент написал, что не может
проверить живой путь, потому что у него нет токена для staging за Access. В хранилище лежал
файл с ровно этим проксирующим секретом, плюс токен провайдера с доступом к логам той самой
функции, которые и отвечали на вопрос. Не хватало не доступа, а двадцати секунд.

**Перед тем как написать «проверить не могу» — выполнить и привести вывод:**

```bash
python ~/.claude/scripts/access_inventory.py <хост|провайдер|сервис>
```

Печатает только ИМЕНА переменных и файлов, никогда значения (значения грузит тот процесс,
которому они нужны — политика `secrets-as-data`). Ищет по обоим хранилищам.

- **Нашлось** → проверка возможна, отказ отменяется, делаем.
- **Не нашлось** → это ЗАДАЧА, а не вердикт: расширить скоуп, взять другую креду, или назвать
  ровно ту, которой не хватает, чтобы её выдали. Формулировка «не смогла, доступа нет» без
  этого вывода — недоделанная работа, а не честная оговорка (см. P11).

Тот же порядок применим к правам вообще, не только к секретам: 403 из-за нехватки нашего
собственного scope — это чинибельный блокер, а не неопределённый результат.
