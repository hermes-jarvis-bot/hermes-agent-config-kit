...
def build_report(evidence: dict[str, Any], context: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *file_lines(context.get("reproduction_files"), "No reproduction files listed."),
        ...
        f"| Relevant-check scope | — | {cell(evidence.get('relevant_scope'))} |",
        f"| Scope rationale | — | {cell(evidence.get('scope_rationale'))} |",
        "",
        ...
        *bullets(context.get("notes"), "No additional notes."),
        ...
        "---",
        "",
        "Generated by `$bug-reproducer`. A fix is proven only by the same red-to-green reproducer plus a declared proportionate scope: a captured additional relevant check or an explicit targeted-only rationale.",
        "",
    ]
    return "\n".join(lines)
